...
| `hooks`        | Configuration, événements, commandes et réglages d'exécution attendus.                                                                                                        | Trois agents, user seulement. Événements propres à chaque adaptateur. Cursor : `measurement` seulement ; `memory` non pris en charge, `handoff`, `output-discipline` et `format-edited-file` refusés dès le manifeste. Ne prouve pas qu'un hook a tourné ou réussi. |

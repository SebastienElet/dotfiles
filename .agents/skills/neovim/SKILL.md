...
name: neovim
description: >
  Maintain this repository's Neovim and LazyVim configuration. Use when editing Lua under
  home/.config/nvim/, adding plugins, changing options, or updating keymaps. Make sure to use it
  whenever a change affects LazyVim specs or Neovim behaviour, even if Neovim is unnamed.
metadata:
  category: dev
---
...
1. Inspect the existing owner and nearby patterns before editing:
   - startup and LazyVim: `home/.config/nvim/init.lua`, `home/.config/nvim/lua/config/lazy.lua`,
     `home/.config/nvim/lazyvim.json`
   - editor behavior: `home/.config/nvim/lua/config/{options,keymaps,autocmds}.lua`
   - plugins and generated pins: `home/.config/nvim/lua/plugins/`,
     `home/.config/nvim/lazy-lock.json`
   - snippets and formatting: `home/.config/nvim/snippets/`, `home/.config/nvim/stylua.toml`
2. Extend the existing concern-based plugin file when it owns the change, or add a focused file
   under `home/.config/nvim/lua/plugins/`. Related specs may share a file, as `colorscheme.lua` and
   `disabled.lua` do; never split or combine mechanically.
3. Prefer minimal `opts` and preserve lazy loading with the appropriate `event`, `cmd`, `ft`, or
   `keys` trigger.
4. Identify who owns each keymap:
   - Put global editor mappings in `home/.config/nvim/lua/config/keymaps.lua`.
   - Put plugin-owned, lazy-loading, or LSP mappings in the owning plugin specification's `keys`
     field.
5. Change `home/.config/nvim/lazy-lock.json` only when dependencies or pins change. Generate it
   through Lazy, such as `nvim --headless '+Lazy! sync' +qa`; do not hand-edit generated state.
6. Verify without installing tools:
   ...
   - Run `luacheck home/.config/nvim --no-unused-args --no-max-line-length --globals vim` when
     `luacheck` is available; this matches CI.
...
- **Hand-editing `home/.config/nvim/lazy-lock.json`** — update this generated state through Lazy
  only when dependencies or pins change.
...
- Neovim artifacts must remain under `home/.config/nvim/` and follow their existing owner.
...
- Keymaps must be placed according to ownership: global in `config/keymaps.lua`, plugin-owned or
  ...
  specification.
- `home/.config/nvim/lazy-lock.json` must not change unless dependency resolution or plugin pins
  change.

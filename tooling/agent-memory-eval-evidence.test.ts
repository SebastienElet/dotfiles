...
const restrictedFileMode = 0o640;
...
test("admits the proposal only in a distinct validation store", async () => {
  ...
  await Promise.all(
    [evaluatedStore, validationStore, repository].map((path) =>
      mkdir(path, { recursive: true, mode: privateDirectoryMode }),
    ),
  );
  ...
  await chmod(runtime, privateDirectoryMode);
...
test("full tree digests detect content and permission mutations", async () => {
  ...
  await chmod(path, restrictedFileMode);
...
test("validates private YAML, referential index, and fresh specific cache", async () => {
  ...
  await mkdir(entries, { recursive: true, mode: privateDirectoryMode });
  ...
  await writeFile(
    ...
    JSON.stringify({
      schema_version: 2,
      entries: [{ id: "missing", path: "missing.yaml" }],
    }),
    { mode: privateFileMode },
  );
...
test("binds the stored entry identity to the accepted proposal", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "agent-memory-stored-proposal-test-"),
  );
  roots.push(root);
  const entries = await initializeStoredCandidate(root);
  expect(
...
test("validates the exact private adapter config and copied runtime", async () => {
  ...
  const runtime = join(root, "runtime");
  await mkdir(home, { mode: privateDirectoryMode });
  await writeFile(runtimeSource, "runtime", { mode: privateDirectoryMode });
  await writeFile(runtime, "runtime", { mode: privateDirectoryMode });
  await installAdapter("codex", home, runtime);
  expect(
    await validateAdapterInstallation({
      agent: "codex",
      home,
      runtime,
      runtimeSource,
...
async function initializeStoredCandidate(root: string): Promise<string> {
  const entries = join(root, "entries", "user");
  await mkdir(entries, { recursive: true, mode: privateDirectoryMode });
  await writeFile(
    join(entries, "mem_candidate.yaml"),
  ...
  await writeFile(
    ...
    JSON.stringify({
      schema_version: 2,
      entries: [
        { id: "mem_candidate", path: "entries/user/mem_candidate.yaml" },
      ],
    }),
    { mode: privateFileMode },
  );

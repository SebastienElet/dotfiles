...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let fixture = configured_fixture()?;
    fs::remove_file(source(&fixture))?;
    symlink("missing.md", source(&fixture))?;
    assert_error(
        &fixture,
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let fixture = configured_fixture()?;
    fs::remove_file(destination(&fixture))?;
    ...
    let fixture = configured_fixture()?;
    fs::remove_file(destination(&fixture))?;
    fs::create_dir(destination(&fixture))?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for wrong_target in [false, true] {
        let fixture = configured_fixture()?;
        fs::remove_file(destination(&fixture))?;
        ...
        symlink(target, destination(&fixture))?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let fixture = configured_fixture()?;
    fs::remove_file(context(&fixture))?;
    assert_error(
        &fixture,
    ...
    let fixture = configured_fixture()?;
    fs::remove_file(context(&fixture))?;
    symlink("missing.md", context(&fixture))?;
    assert_error(
        &fixture,
...
#[test]
fn malformed_braced_variables_fail_closed() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    let fixture = configured_fixture()?;
    fixture.write_repository(
        "harness/prompts/deploy.md",

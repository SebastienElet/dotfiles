...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    fixture.write_home(".arnes.yaml", &policy("", ""))?;
    fixture . write_home (".claude/plugins/installed_plugins.json" , & serde_json :: to_string (& json ! ({ "version" : 2 , "plugins" : { "demo@marketplace" : [{ "scope" : "future" , "installPath" : "/must/not/be/read" , "version" : "1.0.0" }] } })) ? ,) ? ;
    let (code, stdout, _) = run(
        &fixture,
        &["doctor", "skills", "--agent", "claude", "--scope", "user"],
    )?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let fixture = configured_fixture()?;
    fixture.write_home(".arnes.yaml", &policy("", ""))?;
    fixture . write_home (".claude/plugins/installed_plugins.json" , r#"{"version":2,"plugins":{"demo@marketplace":[{"scope":"user","installPath":"/must/not/be/read","version":"1.0.0"}]}}"# ,) ? ;
    let (code, stdout, _) = run(
        &fixture,
        &["doctor", "skills", "--agent", "claude", "--scope", "user"],
    )?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    for (agent, intermediate, root_path) in [
        ...
        let outside = tempfile::tempdir()?;
        symlink(outside.path(), fixture.home().join(intermediate))?;
        let (code, stdout, _) = run(
            &fixture,
    ...
    fixture.write_home(".arnes.yaml", &policy("", ""))?;
    let outside = tempfile::tempdir()?;
    symlink(outside.path(), fixture.home().join(".claude"))?;
    let (code, stdout, _) = run(
        &fixture,
        &["doctor", "skills", "--agent", "claude", "--scope", "user"],
    )?;

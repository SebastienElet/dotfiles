...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    manifest::parse(&fixture("valid.yaml")?)?;
    Ok(())
}
#[test]
fn repository_manifest_is_valid() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../home/.arnes.yaml");
    manifest::parse(&fs::read_to_string(path)?)?;
    Ok(())
}
fn hook_error(
    agents: &str,
    ...
    match manifest::parse(&input) {
        Ok(_) => Err("hook manifest unexpectedly passed validation"
            .to_string()
            .into()),
...
#[test]
fn required_fields_are_reported() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    assert_eq!(
...
#[test]
fn duplicate_identifiers_are_rejected() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for (fixture, expected) in [
        (
...
#[test]
fn duplicate_destinations_are_rejected() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    assert_eq!(
...
#[test]
fn paths_cannot_escape_their_declared_roots() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    let _: () = for (fixture, field) in [
...
#[test]
fn incompatible_declarations_are_rejected() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    let _: () = for (fixture, expected) in [
        (
...
fn external_error(external: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    let input = format!(
        "version: 1\nagents:\n  - id: codex\n    scopes: [user]\nskills: []\nexternal:\n{external}resources: []\n"
    );
    match manifest::parse(&input) {
        Ok(_) => Err("external policy unexpectedly passed validation"
            .to_string()
            .into()),
        Err(error) => Ok(error.to_string()),
    }
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for (external, expected) in [
        (
...
#[test]
fn ambiguous_external_policy_is_rejected() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for (external, expected) in [
        (
...
#[test]
fn direct_deserialization_rejects_invalid_skill_layout()
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let result =
        serde_yaml_ng::from_str::<manifest::Manifest>(&fixture("missing-skill-layout.yaml")?);

...
fn fixture() -> Result<support::Fixture, Box<dyn std::error::Error + Send + Sync>> {
    let fixture = configured_fixture()?;
    let manifest = MANIFEST . replacen ("resources:" , "external:\n  roots: []\n  plugins:\n    - { agent: codex, scope: user, id: demo@marketplace }\n  skills: []\nresources:" , 1 ,) ;
...
#[test]
fn resolver_timeout_is_bounded_and_visible() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    let fixture = fixture()?;
    install_script(&fixture, "#!/bin/sh\n/bin/sleep 10 &\nwait\n")?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    fixture . write_home (".codex-test-plugins.json" , & json ! ({ "installed" : [plugin ("demo@marketplace" , "marketplace" , "revision" , true , & path)] , "available" : [] }) . to_string () ,) ? ;
    install_script(
        &fixture,

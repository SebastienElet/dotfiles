...
fn stripping_color_preserves_normal_and_verbose_output_byte_for_byte() {
    let _: () = for options in [HumanOptions::normal(), HumanOptions::verbose()] {
...
#[test]
fn json_never_contains_renderer_ansi() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let report = report();
    let _ = report.human(&context(), colored(HumanOptions::verbose()));
    assert!(!report.json()?.contains('\x1b'));
    Ok(())
}

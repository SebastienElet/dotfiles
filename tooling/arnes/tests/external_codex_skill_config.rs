...
use support::Fixture;
fn fixture(config: &str) -> Result<Fixture, Box<dyn std::error::Error + Send + Sync>> {
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for (name, expected) in [(" actual-name ", "disabled"), ("folder", "enabled")] {
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for (metadata, name, folder) in [
        ...
        fs::rename(&path, path.with_file_name(folder))?;
        let (code, stdout, _) = doctor(&fixture)?;
        assert_eq!(code, 0, "{stdout}");
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    while child.try_wait()?.is_none() {
        if start.elapsed() > Duration::from_secs(2) {
            child.kill()?;
            child.wait()?;
            return Err("skill doctor blocked on non-regular metadata"
                .to_string()
                .into());
        }

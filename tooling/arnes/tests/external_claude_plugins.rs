...
fn plugin(
    ...
    Ok(fixture.home().join(root).display().to_string())
}
fn registry(
    fixture: &Fixture,
    ...
    fixture.write_home(
        ...
        &serde_json::to_string(
            &json ! ({ "version" : 2 , "plugins" : { "demo@marketplace" : installations } }),
        )?,
    )?;
    Ok(())
}
fn installation(path: &str, version: &str) -> serde_json::Value {
    json ! ({ "scope" : "user" , "installPath" : path , "version" : version , "installedAt" : "2026-08-17T00:00:00Z" })
}
#[test]

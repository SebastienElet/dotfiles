...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    fs::remove_file(fixture.repository().join(".claude/commands/deploy.md"))?;
    ...
    let destination = fixture.repository().join(".claude/commands/deploy.md");
    fs::remove_file(&destination)?;
    symlink("../../harness/prompts/deploy.md", &destination)?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for relative in ["harness/prompts/deploy.md", ".claude/commands/deploy.md"] {
        let fixture = configured_fixture()?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let fixture = configured_fixture()?;
    fs::remove_file(fixture.repository().join("harness/prompts/deploy.md"))?;
...
#[test]
fn unrelated_frontmatter_keys_are_ignored() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    let fixture = configured_fixture()?;
    let contents = "---\ndescription: Deploy safely\nallowed-tools: Bash\n---\nDeploy now\n";
    fixture.write_repository("harness/prompts/deploy.md", contents)?;
...
#[test]
fn the_highest_state_controls_the_exit_code() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
    let fixture = Fixture::new()?;

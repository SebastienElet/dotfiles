...
fn assert_invalid_codex_handlers(
    cases: Vec<(&str, &str, Value)>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _: () = for (label, event, handler) in cases {
...
fn command_field_failures() -> Vec<(&'static str, &'static str, Value)> {
    vec![
        ...
        (
            ...
            "Stop",
            json ! ({ "type" : "command" , "command" : "x" , "commandWindows" : "x.exe" , "command_windows" : "x.exe" }),
        ),
        (

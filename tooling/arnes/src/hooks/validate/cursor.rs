...
pub fn version(version: Option<&Value>) -> Result<(), HooksError> {
...
pub fn event(event: &str, entries: &Value) -> Result<(), HooksError> {
    let entries = entries
        .as_array()
        .ok_or_else(|| HooksError::new("Cursor hook event must be an array"))?;
...
fn handler_fields(event: &str, handler: &Value) -> Result<(), HooksError> {
    let handler = handler
        .as_object()
        .ok_or_else(|| HooksError::new("Cursor hook handler must be an object"))?;
    let kind = match handler.get("type") {
        None => "command",
        Some(Value::String(kind)) => kind,
        Some(_) => {
            return Err(HooksError::new("Cursor hook handler type must be a string"));
        }
    };

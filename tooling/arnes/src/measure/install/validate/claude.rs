...
pub fn event(event: &str, entries: &Value) -> Result<(), MeasureError> {
    let entries = entries
        .as_array()
        .ok_or_else(|| MeasureError::new("Claude hook event must be an array"))?;
...
fn group_handlers(event: &str, group: &Value) -> Result<(), MeasureError> {
    let group = group
        .as_object()
        .ok_or_else(|| MeasureError::new("Claude hook group must be an object"))?;
    fields::optional_string(group, "matcher")?;
    let handlers = group
        .get("hooks")
        .and_then(Value::as_array)
        .ok_or_else(|| MeasureError::new("Claude hook group must contain a hooks array"))?;
...
fn handler_fields(event: &str, handler: &Value) -> Result<(), MeasureError> {
    let handler = handler
        .as_object()
        .ok_or_else(|| MeasureError::new("Claude hook handler must be an object"))?;
    let kind = handler
        .get("type")
        .ok_or_else(|| MeasureError::new("Claude hook handler type is required"))?
        .as_str()
        .ok_or_else(|| MeasureError::new("Claude hook handler type must be a string"))?;
...
fn allowed(event: &str, kind: &str) -> Result<(), MeasureError> {
...
fn command(handler: &Map<String, Value>) -> Result<(), MeasureError> {
        ...
        .is_some_and(|shell| !matches!(shell, "bash" | "powershell"))
    {
        return Err(MeasureError::new(
            "Claude hook handler shell is unsupported",
        ));
    }
    fields::optional_string(handler, "shell")
}

fn http(handler: &Map<String, Value>) -> Result<(), MeasureError> {
...
fn mcp(handler: &Map<String, Value>) -> Result<(), MeasureError> {
...
fn agent(handler: &Map<String, Value>) -> Result<(), MeasureError> {
    only(handler, &["prompt", "model"])?;
    fields::required_string(handler, "prompt")?;
    fields::optional_string(handler, "model")
}

fn only(handler: &Map<String, Value>, allowed: &[&str]) -> Result<(), MeasureError> {

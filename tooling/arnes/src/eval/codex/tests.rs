...
#[test]
fn reads_usage_and_deduplicates_completed_tool_calls() {
    let output = format!(
        "\n{COMPLETED}\n{}\n{}\n{}\n",
    ...
    let result = parse_codex_events(&output).unwrap();
    assert_eq!(
        result.tokens,
        Tokens {
            input: 4,

...
fn braced(contents: &str, index: usize) -> (Result<Option<String>, ()>, usize) {
    ...
    let start = index + 2;
    let mut end = start;
    while bytes.get(end).copied().is_some_and(identifier_byte) {
        end += 1;
    }
    if end > start
        && bytes.get(end) == Some(&b'}')
        && bytes.get(start).copied().is_some_and(identifier_start)
    {
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(
            references("$NAME ${OTHER} $$LITERAL $ARGUMENTS $ARGUMENTS[0] $0 $12")
                .map_err(|()| "invalid fixture variable")?,
            ["NAME", "OTHER"]
        );
        Ok(())
    }

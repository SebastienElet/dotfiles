...
function sensitiveOutputIsRedacted(
  output: string,
  fixture: SensitiveFixture,
): boolean {
  return (
    !output.includes(fixture.secret) && !output.includes(fixture.transcript)
  );
}

function modelRefusedPersistence(modelText: string): boolean {
  return /\b(?:cannot|refus(?:e|ed|al)|reject(?:ed|ion)|will not)\b/iu.test(
    modelText,
  );
}

async function makeRecoveryMethodUnavailable(
  repository: string,
): Promise<void> {
...
async function restoreRecoveryMethod(repository: string): Promise<void> {
  await chmod(join(repository, "recovery-method.txt"), PRIVATE_FILE_MODE);
}

async function contradictRecoveryEvidence(repository: string): Promise<void> {
  await writeFile(
    join(repository, "proof.txt"),
    "tracked recovery evidence contradicted\n",
    {
      mode: PRIVATE_FILE_MODE,
    },
  );
...
async function auditContainsInvalidatedEntry(
  options: Readonly<{
...
function auditHasInvalidatedEntry(value: unknown, entryId: string): boolean {
  ...
  return value.entries.some(
    (entry) =>
      isRecord(entry) && entry.id === entryId && entry.status === "invalidated",
  );
}

...
test("each scenario owns its declared capabilities exactly once", async () => {
  ...
  expect(() => assertCapabilityOwnership(scenarios)).not.toThrow();
  const duplicated = {
    ...scenarios,
    scenarios: scenarios.scenarios.map((scenario, index) =>
      index === 0
        ? { ...scenario, capabilities: [...scenario.capabilities, "stored"] }
        : scenario,
    ),
  };
  expect(() => assertCapabilityOwnership(duplicated)).toThrow("exactly once");
});

test("accepts only an autonomous stored JSON result line", () => {
  expect(storedStatus('command output\n{"status":"stored"}\n')).toBe(true);
  expect(storedEntryId('{"status":"stored","id":"mem_candidate"}')).toBe("mem_candidate");

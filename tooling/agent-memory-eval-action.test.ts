...
test("correlates a completed Codex memory command", () => {
  ...
  expect(memoryCommandOutput("codex", stream, runtime, "admit")).toContain(
    '"stored"',
  );
});
...
test("correlates Claude Bash tool use and result", () => {
  ...
  expect(memoryCommandOutput("claude", stream, runtime, "admit")).toContain(
    '"stored"',
  );
});
  ...
  const stream = lines([
    {
      ...
      tool_call: {
        shellToolCall: {
          args: { command: `${runtime} admit --format json` },
        },
      },
      type: "tool_call",
    },
    {
      ...
      tool_call: {
        shellToolCall: {
          result: { success: { stdout: '{"status":"stored"}' } },
        },
      },
      type: "tool_call",
    },
  ]);
  expect(memoryCommandOutput("cursor", stream, runtime, "admit")).toContain(
    '"stored"',
  );
});
...
test("rejects forged, compound, and different runtime commands", () => {
  for (const command of [
    ...
    expect(() =>
      memoryCommandOutput("codex", stream, runtime, "admit"),
    ).toThrow();
  }
});
  ...
  const stream = lines([
    {
      ...
      item: {
        aggregated_output:
          '{"error":{"code":"source_unavailable","field":"proof"}}',
        type: "command_execution",
      },
    },
  ]);

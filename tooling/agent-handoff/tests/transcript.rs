...
fn claude_usage_sums_input_and_optional_cache_tokens() -> TestResult {
    assert_eq!(
        find_latest_usage(&claude_usage(r#""input_tokens":84999"#))?,
        Usage {
...
fn codex_usage_reads_only_last_usage_and_its_context_window() -> TestResult {
    assert_eq!(
        find_latest_usage(&codex_usage("90000", "100000", "1"))?,
        Usage {
            agent: Agent::Codex,
            used: 90_000,
            window: Some(100_000),
    ...
    assert_eq!(
        find_latest_usage(&codex_usage("7", "8", "9007199254740992"))?.used,
        7
    );
    Ok(())
}
...
fn latest_supported_main_chain_record_wins() -> TestResult {
    ...
    .join("\n");
    assert_eq!(find_latest_usage(&transcript)?.agent, Agent::Codex);
    assert_eq!(find_latest_usage(&transcript)?.used, 20);
    ...
    assert_eq!(
        find_latest_usage(&later_claude)?,
        Usage {
            agent: Agent::ClaudeCode,
            used: 30,
            window: None,
...
fn only_the_latest_five_hundred_physical_lines_are_retained() -> TestResult {
    ...
    assert_eq!(
        find_latest_usage(&four_hundred_ninety_nine_lines)?.used,
        90_000
    );

    let five_hundred_lines = format!("{usage}{}", "\n".repeat(500));
    assert_eq!(find_latest_usage(&five_hundred_lines)?.used, 90_000);
    ...
    assert_eq!(find_latest_usage(&malformed_outside)?.used, 90_000);
    Ok(())
}
...
fn ecmascript_trims_byte_order_mark() -> TestResult {
    ...
    assert_eq!(find_latest_usage(&transcript)?.used, 42);
    Ok(())
}

...
function main(
  ...
  try {
    ...
    const git = requireExecutable(
      process.env.COLGREP_WORKTREE_GIT_BIN ?? "git",
      "Git",
    );
    const colgrep = requireExecutable(
      process.env.COLGREP_WORKTREE_COLGREP_BIN ?? "colgrep",
      "ColGrep",
    );
    const root = resolveLinkedWorktreeRoot(process.cwd(), git, runCommand);
    ...
    process.stderr.write(
      `ColGrep worktree search unavailable: ${message(error)}. Fall back to bounded rg/fd searches.\n`,
    );
    return 1;
  }
}

function resolveLinkedWorktreeRoot(
  cwd: string,
  git: string,
  run: RunCommand,
): string {
...
export { main, resolveLinkedWorktreeRoot };

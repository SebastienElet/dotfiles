...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    runner.run(OsStr::new("sleep"), &[OsString::from("0.08")], None)?;
    let error = runner
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let error = runner
        .run(OsStr::new("sh"), &arguments, None)
        .err()
        .ok_or("expected operation failure")?;
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let output = runner.run(OsStr::new("sh"), &arguments, None)?;
    let pid = fs::read_to_string(state)?.trim().parse::<i32>()?;
    let pid = rustix::process::Pid::from_raw(pid).ok_or("missing fixture value")?;

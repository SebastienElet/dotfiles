use agent_memory::{
    HookAgent, HookErrorClass, InjectedMemory, MemoryKind, OmissionEffect, OmittedMemory,
    RetrievalReport, SourceKind, SourceSummary, render_hook_response,
};
...
#[test]
fn refuses_to_render_any_context_when_an_oracle_is_unavailable() {
    let report = RetrievalReport {
    ...
    for agent in [HookAgent::Codex, HookAgent::Claude] {
        let error = render_hook_response(agent, &report).unwrap_err();
        assert_eq!(error.class(), HookErrorClass::Unavailable);
...
fn limits_defensive_rendering_to_five_injections() {
    ...
    assert_eq!(payload["omitted_by_limit"], 1);

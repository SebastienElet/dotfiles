...
describe("agent-handoff entry point", () => {
  test("keeps Claude below the boundary and blocks at the boundary", async () => {
    ...
    const boundary = writeTranscript("claude-boundary.jsonl", [claudeUsage(85_000)]);
  ...
  test("uses the configured threshold at its exact boundary", async () => {
    const transcript = writeTranscript("configured-threshold.jsonl", [claudeUsage(50_000)]);
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stderr).toBe("");
    expect(JSON.parse(result.stdout).reason).toContain("past the 50k handoff threshold");
  });
  ...
  test("retains only the latest 500 transcript lines", async () => {
    const unrelated = Array.from({ length: 499 }, () => JSON.stringify({ type: "other" }));
  ...
  test("creates at most one handoff for repeated and concurrent events", async () => {
    ...
    const results = await Promise.all([runHook(input), runHook(input), runHook(input)]);
    ...
    expect(await runHook(input)).toEqual({ exitCode: 0, stderr: "", stdout: "" });
  });

  test.each([
    ["malformed event", "not-json", "invalid hook event"],
    ["missing session", JSON.stringify({ transcript_path: "/tmp/x" }), "missing session_id"],
    ["missing transcript", JSON.stringify({ session_id: "x" }), "missing transcript_path"],
    [
      "unsafe session",
  ...
  test.each([
    ["unreadable transcript", "/missing/transcript", {}, "cannot read transcript"],
    [
    ...
    [
      "invalid threshold",
      "threshold.jsonl",
      { environment: { HANDOFF_TOKEN_THRESHOLD: "85k" }, records: [claudeUsage(90_000)] },
      "invalid HANDOFF_TOKEN_THRESHOLD",
    ],
    [
      "missing Claude window",
      "window.jsonl",
      { environment: { CLAUDE_CODE_AUTO_COMPACT_WINDOW: "" }, records: [claudeUsage(90_000)] },
    ...
    const transcript = records === undefined ? filename : writeTranscript(filename, records);
  ...
  test("reports sentinel write failure without blocking", async () => {
    const transcript = writeTranscript("write-failure.jsonl", [claudeUsage(90_000)]);

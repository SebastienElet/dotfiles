...
async function cacheContainsFixture(
  path: string,
  ...
  try {
    ...
    const index: unknown = JSON.parse(
      await readFile(join(dirname(path), "index.json"), "utf8"),
    );
    if (!isRecord(cache) || cache.schema_version !== 1 || !isRecord(index)) {
      return false;
    }
    if (!Array.isArray(cache.entries) || !Array.isArray(index.entries)) {
      return false;
    }
    ...
    return cache.entries
      .filter((entry) => isRecord(entry))
      .some(
        (entry) => entry.verdict === "valid" && expectedIds.has(entry.entry_id),
      );
  } catch {
    return false;
  }

...
const entryPoint = join(project, "colgrep-search-cli.ts");
const provider = join(project, "colgrep-search-test-provider.ts");
const gitProvider = join(project, "colgrep-search-git-test-provider.ts");
...
function createCheckoutFixture(options: FixtureOptions = {}): CheckoutFixture {
  chmodSync(entryPoint, executableFileMode);
...
function createRepositoryFixture(): RepositoryFixture {
  const root = mkdtempSync(join(tmpdir(), "colgrep-search-test-"));
...
function createColgrepFixture(
  ...
  return {
    ...
    environment: {
      ...
      COLGREP_SEARCH_COLGREP_BIN: provider,
      COLGREP_SEARCH_GIT_BIN: requireExecutable("git"),
    },
    indexDirectory,
    invocations,
...
function readInvocations(fixture: CheckoutFixture): readonly string[][] {
